...
test("shows 1 total board after loading f5e project", async ({ page }) => {
  await openFileAndGoToBoards(page, F5E_SPM, "F-5E Armament Panel");
  await expect(page.getByText("1 total")).toBeVisible();
});

test("shows board name in sidebar", async ({ page }) => {
  await openFileAndGoToBoards(page, F5E_SPM, "F-5E Armament Panel");
...
test("shows board type (leonardo) in sidebar", async ({ page }) => {
  await openFileAndGoToBoards(page, F5E_SPM, "F-5E Armament Panel");
  await expect(page.getByText("leonardo")).toBeVisible();
});

test("shows board USB product name in sidebar", async ({ page }) => {
  await openFileAndGoToBoards(page, F5E_SPM, "F-5E Armament Panel");
  // "F5E Armament" is the usbProduct value
  await expect(page.getByText("F5E Armament")).toBeVisible();
});

test("shows VID and PID in hex format", async ({ page }) => {
  await openFileAndGoToBoards(page, F5E_SPM, "F-5E Armament Panel");
...
test('"Allocate Identity" button visible for boards with prototype PID', async ({ page }) => {
  await openFileAndGoToBoards(page, F5E_SPM, "F-5E Armament Panel");
  // PID = 1 which is <= 0x000F
  await expect(page.getByRole("button", { name: "Allocate Identity" })).toBeVisible();
});

test("clicking board loads pin map detail panel", async ({ page }) => {
  await openFileAndGoToBoards(page, F5E_SPM, "F-5E Armament Panel");
...
test("pin map detail shows used pin count", async ({ page }) => {
  await openFileAndGoToBoards(page, F5E_SPM, "F-5E Armament Panel");
  await page.getByText("Armament", { exact: true }).click();
...
test("pin map detail shows control labels in used pins table", async ({ page }) => {
  await openFileAndGoToBoards(page, F5E_SPM, "F-5E Armament Panel");
  await page.getByText("Armament", { exact: true }).click();
...
test("clicking Allocate Identity triggers API and shows dirty indicator", async ({ page }) => {
  await openFileAndGoToBoards(page, F5E_SPM, "F-5E Armament Panel");
...
test("multi-board project: selecting different boards shows different pin maps", async ({
  ...
  await openFileAndGoToBoards(page, MULTI_SPM, "Multi-board Demo");
...
test("pin map shows warning for serial pins D0 and D1", async ({ page }) => {
  await openFileAndGoToBoards(page, F5E_SPM, "F-5E Armament Panel");
  await page.getByText("Armament", { exact: true }).click();
...
test("select a board placeholder shows when none selected", async ({ page }) => {
  await openFileAndGoToBoards(page, F5E_SPM, "F-5E Armament Panel");
