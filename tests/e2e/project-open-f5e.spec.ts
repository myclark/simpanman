...
async function openF5eProject(page: import("@playwright/test").Page) {
...
test("opens f5e-armament.spm and shows project name in title bar", async ({ page }) => {
  await openF5eProject(page);
  await expect(page.getByRole("banner").getByText("F-5E Armament Panel")).toBeVisible();
  await expect(page.getByRole("banner").getByText("No project open")).not.toBeVisible();
});

test("Controls toolbar shows 18 controls count", async ({ page }) => {
  await openF5eProject(page);
  // The toolbar span is the first element showing the total count
  await expect(page.getByText("18 controls").first()).toBeVisible();
});

test("panel group row is visible and shows child count", async ({ page }) => {
  await openF5eProject(page);
...
test("can expand panel group to see individual controls", async ({ page }) => {
  await openF5eProject(page);
...
test("can collapse panel group after expanding", async ({ page }) => {
  await openF5eProject(page);
  const groupTd = page
    .locator("tr")
    .filter({ hasText: "Armament Panel" })
...
test("no dirty indicator when project is first opened", async ({ page }) => {
  await openF5eProject(page);
...
test("expanded grid shows correct kinds for controls", async ({ page }) => {
  await openF5eProject(page);
  const groupTd = page
    .locator("tr")
    .filter({ hasText: "Armament Panel" })
...
test("can open f5e project via empty-state Open button", async ({ page }) => {
  const chooser = page.waitForEvent("filechooser");
  await page.getByRole("button", { name: "Open .spm File" }).click();
  const fc = await chooser;
  await fc.setFiles(F5E_SPM);
  await expect(page.getByRole("banner").getByText("F-5E Armament Panel")).toBeVisible();
});
